...
|------|------|-------------|---------|
| [Agenda](addons/agenda@beatlink/) | widget | A schema-driven, multi-profile task/agenda system for TriliumNext, in three widgets that share one configuration: Overview (a right-pane widget whose search/filter/sort/prefix/color rules re-file the active profile&#x27;s matching notes under a shared overview note shown as a built-in collection view, exporting an iCal feed, and the Agenda Editor page that edits the config), Task (a right-pane editor for a task&#x27;s start/due dates, duration, recurrence, and quick actions), and My Day (a note-detail countdown timer that files due tasks and sends notifications while your My Day note is open). The config lives in one settings note (schema.json/config.json) tagged #agendaConfig; every widget discovers and reads it at runtime. | 1.4.0 |
...
| [Form Controls](addons/libformcontrols@beatlink/) | library | Reusable Preact form-control components for TriliumNext widget UIs: a datetime-local input, a number input, a time input, a toggle button (checkbox styled as a pill), a labeled collapsible checkbox group, a curated color-swatch picker, and a collapsible section. Each control is exported separately so a consumer pulls in only the ones it uses. | 1.2.0 |
...
| [Trilium Addon Manager](addons/trilium-addon-manager@beatlink/) | widget | This addon allows for the easy installation, removal and updating of Trilium addons from any manifest URL or catalog. | 5.3.0 |
