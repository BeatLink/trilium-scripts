...
| [Form Controls](addons/libformcontrols@beatlink/) | library | Reusable Preact form-control components for TriliumNext widget UIs: a toggle button (checkbox styled as a pill), a labeled collapsible checkbox group, a curated color-swatch picker, and a collapsible section. Each control is exported separately so a consumer pulls in only the ones it uses. | 1.3.0 |
