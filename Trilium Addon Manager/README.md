# Trilium Addon Manager (TAM)

![Screenshot](./image.png)

## Overview

This addon allows for the easy installation, removal and updating of Trilium addons from GitHub repositories.

## Features

* Manage Multiple Addon Repositories
  * Add Repository
  * Remove Repository
* Manage Addons
  * Show addon information
  * Install addons
  * Update addons
  * Uninstall addons
* Allow installation of multiple addons from one repository (monorepo architecture)
* [PLANNED] Dependency Management for addons
* [PLANNED] Persistence of addon data across updates and reinstalls

## Setting Up an Addon Repository

For setting up an addon repository to publish your own addons, see [./Setting Up an Addon Repository.md](./Setting%20Up%20an%20Addon%20Repository.md)

## Installing Trilium Addon Manager

To install, download and import the addon manager zip file from the releases page. Then in the settings JSON, set the password field to your Trilium web password.
