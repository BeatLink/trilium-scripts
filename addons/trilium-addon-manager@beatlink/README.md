...
Since a library is never installed on its own, there's also no reason for its root note to clutter the same top-level "Addons" list a user actually browses — `syncAddon` roots `type: "library"` addons under a separate `Addons > Libraries` folder instead (resolved via a `libraryRoot` relation on `lib-tam`, mirroring `addonRoot`'s existing pattern), falling back to the ordinary `Addons` anchor if that relation isn't resolvable for some reason. A library's own tree still needs *some* stable, addon-external parent to be created under during its independent `resolveNotes` pass, before any dependent has cloned anything from it — this folder exists purely to be that anchor without being something a user has to scroll past. A library synced under an older TAM version (before this folder existed, so its root ended up under `Addons` directly) gets migrated the next time it syncs: `reconcileNoteParenting` accepts an optional `staleAnchorNoteId` for exactly this case, detaching the old `Addons` branch even though it isn't tagged with the library's own `#TAMFILEID` prefix (the usual rule for what a sync is allowed to detach).

`deleteAddon` is branch-scoped for the same reason a library can end up parented in more than one place: it never calls `note.deleteNote()` directly on an addon's root. Instead, `detachAddonOwnedBranches` walks every note in the addon's own stored manifest and, for each of its current parents, detaches the branch unless that parent is clearly tagged as belonging to a *different* addon (its own `#TAMFILEID` prefix says so) — an addon's own anchor folder, its own same-addon multi-parent clones, and untagged/manual parents are all fair game to detach; another addon's clone of an exported note is not. A note only actually disappears once none of its parents are left. This doesn't rely on any assumption about how Trilium's own delete cascade treats a note with another live parent elsewhere — every note is checked and detached individually, so a library still depended on by another installed addon is provably safe to leave alone during uninstall, not just probably safe.
